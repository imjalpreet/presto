        ...
        implements ConnectorPlanOptimizer
{
            ...
            Optional<ConnectorTableLayoutHandle> currentLayoutHandle)
    {
        ...
        if (currentLayoutHandle.isPresent()) {
            entireColumnDomain = entireColumnDomain.intersect(((IcebergTableLayoutHandle) (currentLayoutHandle.get())).getPartitionColumnPredicate());
        }
        ...
        TupleDomain<ColumnHandle> unPartitionColumnPredicate = TupleDomain.withColumnDomains(Maps.filterKeys(constraint.getSummary().getDomains().get(), not(Predicates.in(getPartitionKeyColumnHandles(icebergTable, typeManager)))));

        TupleDomain<Subfield> domainPredicate = withColumnDomains(ImmutableMap.<Subfield, Domain>builder()
                .putAll(unPartitionColumnPredicate
        ...
        return new ConnectorPushdownFilterResult(
                metadata.getTableLayout(
                        ...
                        new IcebergTableLayoutHandle(
                                getPartitionKeyColumnHandles(icebergTable, typeManager),
                                domainPredicate,
                                remainingExpression,
                                predicateColumns,
